...
calc_mean_curvature()
{
	...
	for(v_it = mesh_.vertices_begin(); v_it != v_end; ++v_it)
	{
		...
		v = mesh_.property(vweight_,v_it);
		m *= v;
		mesh_.property(vcurvature_, v_it.handle()) = 0.5 * m.length();
	}
}
...
calc_uniform_mean_curvature()
{
	...
	for(v_it = mesh_.vertices_begin(); v_it != v_end; ++v_it)
	{
		...
		m /= n_neighbours;
		laplace = m - mesh_.point(v_it);
		double length = 0.5 * laplace.length();
		mesh_.property(vunicurvature_, v_it.handle()) = length;
	}
}
...
calc_gauss_curvature()
{
	...
	for(v_it = mesh_.vertices_begin(); v_it != v_end; ++v_it)
	{
		...
		Mesh::Point firstPoint, PrevPoint, NextPoint;
		angles = 0;
		for(vv_it = mesh_.vv_iter(v_it); vv_it; ++vv_it, ++i)
		{
			if(i == 0)
			{
				firstPoint = mesh_.point(vv_it);
				PrevPoint = mesh_.point(vv_it);
			}
			else
			{
				NextPoint = mesh_.point(vv_it);
				cos_angle = dot(NextPoint - p, PrevPoint -p) / ((NextPoint - p).length() * (PrevPoint -p).length());
		...
		cos_angle = dot(firstPoint - p, PrevPoint -p) / ((firstPoint - p).length() * (PrevPoint -p).length()); 
		angles += acos(cos_angle);
		
		double v = mesh_.property(vweight_,v_it);
		double gauss = 2 * v * (2 * M_PI - angles);
		mesh_.property(vgausscurvature_, v_it) = gauss;
	}
}
...
calc_triangle_quality()
{
	...
	Mesh::Scalar numerator;

	for(f_it = mesh_.faces_begin(); f_it != f_end; ++f_it)
	{
		Mesh::Scalar ratio = FLT_MAX;
		cfvIt = mesh_.fv_begin(f_it.handle());
		v0 = mesh_.point(cfvIt.handle());cfvIt++;
		v1 = mesh_.point(cfvIt.handle());cfvIt++;
		v2 = mesh_.point(cfvIt.handle());cfvIt++;

		v0v1 = v1 - v0;
		v0v2 = v2 - v0;
		v1v2 = v2 - v1;

		float v0v1Len = v0v1.length();
		float v0v2Len = v0v2.length();
		float v1v2Len = v1v2.length();
		denom = 2 * (cross(v0v1, v0v2).length());
		numerator =  v0v1Len * v0v2Len * v1v2Len;
		if(denom > FLT_MIN)
		{
			circum_radius_sq = numerator /denom;
			if((v0v1Len < v0v2Len) && (v0v1Len < v1v2Len))
				min_length_sq = v0v1Len;
			else if((v0v2Len < v0v1Len) && (v0v2Len < v1v2Len))
				min_length_sq = v0v2Len;
			else
				min_length_sq = v1v2Len;
			ratio = circum_radius_sq / min_length_sq;
		}

		mesh_.property(tshape_,f_it.handle()) = ratio;
	}
