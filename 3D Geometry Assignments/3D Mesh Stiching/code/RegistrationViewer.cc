...
)
{
	...
	for(uint i = 0; i < _pts.size(); i+=3)
	{
		int addPt = 1;
		for(uint j = 0; j < indeces.size(); j++)
		{
			if(length(_pts[i] - _pts[indeces[j]]) < subsampleRadius)
			{
				addPt = 0;
				break;
			}
...
)
{
	...
	{
		distance.push_back(length(targetCandidatePts[i] - srcCandidatePts[i]));
	}
	...
	if((dSize % 2) != 0)
		medianElement = distance[(dSize-1)/2];
	else
		medianElement = (distance[dSize/2] + distance[(dSize/2) - 1]) /2;

	for(int i = 0; i < srcCandidatePts.size(); i++)
	{
		Vector3d srcPoint = srcCandidatePts[i];
		Vector3d targetPoint = targetCandidatePts[i];
		Vector3d srcNormal = srcCandidateNormals[i];
		Vector3d targetNormal = srcCandidateNormals[i];

		float cosAngle = dot_product(srcNormal, targetNormal)/ (length(srcNormal) * length(targetNormal));
		float l = length(targetPoint - srcPoint); 
		if((l < distMedianThresh * medianElement) && (cosAngle > cos(normalCompatabilityThresh * 3.14159265 / 180.0)))
		{
			_src.push_back(srcPoint);
			_target.push_back(targetPoint);
			_target_normals.push_back(targetNormal);
		}
	}
